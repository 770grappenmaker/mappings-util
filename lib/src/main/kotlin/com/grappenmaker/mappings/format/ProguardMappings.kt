...
public data object ProguardMappingsFormat : MappingsFormat<ProguardMappings> {
    ...
    private fun String.asMapping(): List<String> {
        val result = split(" -> ")
        if (result.size != 2) ctx.parseError("expected mapping (2 values, got ${result.size}): $this")
        return result
    }
    ...
    private class MappingState : ProguardState {
        ...
        override fun update(): ProguardState {
            if (ctx.line.startsWith(INDENT)) ctx.parseError("Invalid top level indent")
            return ClassState(this, ctx.line.removeSuffix(":").asMapping().map { it.asResourceName() })
        }
    ...
    private class ClassState(
        ...
        context(ctx: LineAndNumber)
        override fun update(): ProguardState {
            if (!ctx.line.startsWith(INDENT)) {
                end()
                return owner.update()
            }

            val trimmed = ctx.line.trim()
