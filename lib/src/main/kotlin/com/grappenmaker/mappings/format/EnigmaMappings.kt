...
public data object EnigmaMappingsFormat : MappingsFormat<EnigmaMappings> {
    ...
    private class MappingState : EnigmaState.ClassHolding {
        ...
        override fun update(): EnigmaState {
            val ident = ctx.line.countStart()
            val (type, parts) = ctx.line.prepare()

            if (ident != 0) ctx.parseError("Invalid indent top-level")
            if (type != "CLASS") ctx.parseError("Invalid top-level member type $type")
    ...
    private class ClassState(
        ...
        override fun update(): EnigmaState {
            ...
            return when (type) {
                ...
                else -> ctx.parseError("Invalid class member type $type")
            }
        }
    ...
    private class FieldState(
        ...
        override fun update(): EnigmaState {
            ...
            val (type, parts) = ctx.line.prepare()
            if (type != "COMMENT") ctx.parseError("Invalid field member type $type!")
            comments += parts.join()
            ...
            return when (type) {
                ...
                "ARG" -> {
                    ...
                    parameters += MappedParameter(
                        names,
                        index.toIntOrNull() ?: ctx.parseError("Invalid parameter index $index")
                    )

                    dummyArgState
                }

                else -> ctx.parseError("Invalid method member type $type")
            }
        }
    ...
    private class ArgState(private val owner: MethodState, private val indent: Int) : EnigmaState {
        ...
        override fun update(): EnigmaState {
            ...
            val (type) = ctx.line.prepare()
            if (type != "COMMENT") ctx.parseError("Invalid arg member type $type")
